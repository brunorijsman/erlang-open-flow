...
header_bin() ->
    << ?OF_VERSION_2 : 8,            % Version
       ?OF_MESSAGE_TYPE_HELLO : 8,   % Type
       0 : 16,                       % Length
       0 : 32 >>.                    % Xid

header_rec() ->
    #of_header{version = ?OF_VERSION_2,
...
error_bin() ->
    <<?OF_ERROR_TYPE_BAD_MATCH               : 16,
      ?OF_ERROR_CODE_BAD_MATCH_BAD_WILDCARDS : 16 >>.

error_rec() ->
    #of_error{type = ?OF_ERROR_TYPE_BAD_MATCH,
              code = ?OF_ERROR_CODE_BAD_MATCH_BAD_WILDCARDS,
              data = <<>>}.

error_with_data_bin() ->
    <<?OF_ERROR_TYPE_BAD_ACTION              : 16,
      ?OF_ERROR_CODE_BAD_ACTION_BAD_OUT_PORT : 16,
      5, 4, 3, 2, 1 >>.

error_with_data_rec() ->
    #of_error{type = ?OF_ERROR_TYPE_BAD_ACTION,
...
echo_request_rec() ->
    #of_echo_request{data = <<>>}.

echo_request_with_data_bin() ->
    <<3, 3, 3, 4, 4, 4>>.

echo_request_with_data_rec() ->
    #of_echo_request{data = <<3, 3, 3, 4, 4, 4>>}.
...
echo_reply_with_data_rec() ->
    #of_echo_reply{data = <<3, 3, 3, 4, 4, 4>>}.
...
experimenter_rec() ->
    #of_experimenter{experimenter_id = 1,
                     data            = <<>>}.
...
experimenter_with_data_rec() ->
    #of_experimenter{experimenter_id = 1234,
                     data            = <<99, 99, 88, 88>>}.
...
features_reply_rec() ->
    #of_features_reply{data_path_id = 123456789,
                       n_buffers    = 5000,
                       n_tables     = 50,
...
capabilities_rec() ->
    #of_capabilities{flow_stats   = false,
                     table_stats  = false,
                     port_stats   = false,
                     group_stats  = true,
                     ip_reasm     = false,
...
port_features_rec() ->
    #of_port_features{half_duplex_10_mbps  = true,
